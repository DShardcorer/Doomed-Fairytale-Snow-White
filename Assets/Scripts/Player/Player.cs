...
public class Player : Entity, ILifecycle<PlayerManager>, IFixedUpdatable, IUpdatable
{
    ...
    public void Initialize(PlayerManager parent)
    {
        ...
        //State creation
        _playerIdlingState = new PlayerIdlingState(AnimationStateHelper.IS_IDLING);
        _playerIdlingState.Initialize(this);

        _playerMovingProperties = new PlayerMovingProperties();
        _playerMovingState = new PlayerMovingState(_playerMovingProperties, AnimationStateHelper.IS_MOVING);
        _playerMovingState.Initialize(this);

        _playerDashingProperties = new PlayerDashingProperties();
        _playerDashingState = new PlayerDashingState(_playerDashingProperties, AnimationStateHelper.IS_DASHING);
        _playerDashingState.Initialize(this);


        _playerAttackingProperties = new PlayerAttackingProperties();
        _playerAttackingState = new PlayerAttackingState(_playerAttackingProperties, AnimationStateHelper.IS_ATTACKING);
