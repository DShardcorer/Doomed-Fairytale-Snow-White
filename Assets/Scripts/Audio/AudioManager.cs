...
namespace AudioSystem
{
    public class AudioManager : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            ...
            voicePool = CreateAudioPool(voicePoolSize, voiceParent, "Voice_", audioMixer.FindMatchingGroups("Voice")[0]);
        ...
        private List<PooledAudioSource> CreateAudioPool(int size, Transform parent, string prefix, AudioMixerGroup mixerGroup)
        {
        ...
        public void StopMusic(bool fade = true, float fadeDuration = -1)
        {
            ...
            {
                float actualFadeDuration = fadeDuration > 0 ? fadeDuration : defaultFadeDuration;
                musicFadeCoroutine = StartCoroutine(FadeMusicVolume(musicSource.volume, 0, actualFadeDuration, stop: true));
            }
            else
            {
                musicSource.Stop();
        ...
        public void StopAmbience(bool fade = true, float fadeDuration = -1)
        {
            ...
            {
                float actualFadeDuration = fadeDuration > 0 ? fadeDuration : defaultFadeDuration;
                ambienceFadeCoroutine = StartCoroutine(FadeAmbienceVolume(ambienceSource.volume, 0, actualFadeDuration, stop: true));
            }
            else
            {
                ambienceSource.Stop();
        ...
        public string PlaySFX(AudioClip clip, float volume = 1f, float pitch = 1f, bool loop = false, Vector3? position = null)
        {
        ...
        {
            if (string.IsNullOrEmpty(soundId) || !activeAudioSources.TryGetValue(soundId, out PooledAudioSource pooledSource))
                return false;
        ...
        /// </summary>
        public string PlayRandomSFX(AudioClip[] clips, float volume = 1f, float pitchMin = 0.9f, float pitchMax = 1.1f, bool loop = false, Vector3? position = null)
        {
        ...
        /// </summary>
        public string PlaySFXFromResources(string resourcePath, float volume = 1f, float pitch = 1f, bool loop = false, Vector3? position = null)
        {
            AudioClip clip;
        ...
        private void ConfigureAudioSource(AudioSource source, AudioClip clip, float volume, float pitch, bool loop, Vector3? position)
        {
            source.clip = clip;
